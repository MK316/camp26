...
with tab4:
    ...
    st.caption("주관식 문항입니다. 'No Response'는 제외됩니다. 아래는 키워드 빈도, 그룹별 비교, 공동출현 네트워크를 포함합니다.")
    ...
    if open_s.empty:
        st.warning("현재 필터 조건에서 B4 주관식 응답이 없습니다.")
        st.stop()
    ...
    # (B) 응답 길이 분포
    # ---------------------------
    st.subheader("🧾 응답 길이 분포")
    len_df = pd.DataFrame({
        "text": open_s,
        "length_words": open_s.map(lambda x: len(re.findall(r"\S+", x)))
    })
    fig_len = px.histogram(len_df, x="length_words", nbins=20, title="단어 수 기준 분포")
    fig_len.update_layout(height=420, margin=dict(l=20, r=20, t=60, b=20),
                          xaxis_title="단어 수(대략)", yaxis_title="응답 수")
    ...
    top_n = st.slider("Top 키워드 개수", min_value=10, max_value=60, value=25, step=5, key="b4_topn")
    ...
    if len(all_tokens) == 0:
        ...
        fig_kw.update_layout(height=560, margin=dict(l=20, r=20, t=60, b=20),
    ...
    st.caption("필터로 선택된 데이터 안에서 그룹별로 상위 키워드를 비교합니다.")
    group_col = st.selectbox(
        "그룹 기준 선택",
        ["Field_Group", "Year_Level"],
        index=0,
        key="b4_groupcol"
    )
    ...
    # 그룹별 토큰 집계
    tmp_df = fdf.copy()
    tmp_df["__open__"] = tmp_df[COL_B4].astype(str).map(clean_text)
